...
### Beta v0.1.76 - Performance Boost & Enhanced Tooling (June 7, 2025)

This version brings significant performance improvements, LLVM integration, and logging function updates. The installation process has been revamped to be faster and more lightweight, and the compiler now includes experimental LLVM support for better optimizations.

**Key Improvements:**
- **Performance & Installation**: Completely revamped installation process with reduced footprint
- **LLVM Integration**: Experimental support for LLVM-based compilation
- **Logging Updates**: Renamed logging functions to avoid conflicts (`info`→`infolog`, `warn`→`warnlog`, etc.)
- **VS Code Extension**: Improved syntax highlighting and code completion
- **Memory Management**: Reduced memory usage and optimized performance

**Detailed Changelog:**
- [Markdown Version (beta_v0.1.76.md)](beta_v0.1.76.md)

---

### Beta v0.1.75 - Library Call Update & Namespace Notation (June 5, 2025)

This version introduces the modern namespace notation (`lib::function`) for library calls, providing full parity with the classic bracket notation. The parser and compiler have been significantly enhanced to handle complex library calls robustly. The overall library experience is modernized with consistent metadata extraction for documentation, including difficulty and version badges. Error handling and messaging for library calls have also been improved.

**Key Improvements:**
- **Namespace Notation**: Introduced `lib::function` syntax for all library calls.
- **Full Parity**: Both namespace and bracket notations are fully supported and interchangeable.
- **Enhanced Parser/Compiler**: Improved robustness for complex, nested, and chained library calls.
- **Modernized Library Experience**: Consistent metadata (difficulty, version badges) across docs and sidebars.
- **Improved Error Handling**: Clearer error messages for invalid library calls.
- **Backward Compatibility**: Bracket notation (`Library[function]`) remains supported until beta v0.1.80, after which it will be removed.

**Detailed Changelog:**
- [Markdown Version (beta_v0.1.75.md)](beta_v0.1.75.md)
